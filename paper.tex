...
Additionally, the Eclipse Usage Data Collector (UDC) interaction data allows us to dig further into more specific subjects resulting in the following complementary research questions:
\begin{itemize}
\item \textbf{RQ4:} How is the developer's performance during the recovery time after an interruption? 
\item \textbf{RQ4:} What events are more common during recovery time?
\end{itemize}
...
\textbf{Structure of the paper.} We start with a literature review of studies of work fragmentation in Section 2. This section also contains a review of studies performed with the Mylyn dataset that we use. In Section 3, we describe the Mylyn dataset in further detail, highlighting potential issues in the operational data that we use in the analysis. In Section 4, we describe the measurements used in this study, and detail the processing of the data that is applied to alleviate the issues described in Section 3. The same section shows how we convert interaction traces to multivariate time series representing development sessions, and present the metrics in the analysis. In Section 5, we answer our first research question: we find an inverse relationship between number of interruptions and our three productivity indicators. In Section 6, we answer our second research question, finding that this relationship is more pronounced for development sessions with at least one longer interruption. In Section 7 we answer our third research question and find that our productivity indicators are indeed lower in the vicinity of interruptions; we further find three patterns of interruptions with different characteristics. In Section 8 we describe the Usage Data Collector dataset and give detail on the selection and transformation procedures. In Section 9, we compare the Mylyn data with Usage Data Collector by executing the same processes to obtain the above results and we find that all of the previous conclusions agree with this new data. In Section 10 we answer our fourth research question, finding a positive evolution of productivity during the recovery time and a relation with the effect of the interruption. In Section 11, we answer our fifth research question finding differences on the activities performed after a positive or negative interruption, judging by the type of events executed. We close the paper by discussing the threats to validity of this study in Section 12, before concluding in Section 13.
...
The Usage Data Collector dataset is a large collection of information about interaction data of Eclipse's users. It is publicly available and has been used on many subjects around mining interaction data. On refactoring activities, Murphy-Hll et al. used this data to understand how programmers are using refactoring tools analyzing patterns on refactoring practices, finding that most programmers don't make deep use of this tools, leaving untouched most of the configuration parameters and performing manually most of the refactoring  \cite{MPB12}. Liu et al. identified refactoring tactics of the type data floss and root canal, finding that the majority of them are floss refactoring and detecting differences on activities performed depending on the type of the refactoring \cite{LGN12}. Sahin et al. analyzed the energy usage changes after refactoring and used the UDC data to identify the most common refactoring events and selected those that fulfill the requirements of being common and cause structural changes \cite{SPC14}. 

On coding tools and strategies, Murphy-Hill et al. investigated on command recommendation for software developers depending on past history of command usage and tested a set of algorithms using UDC data to obtain CVS and editing commands, getting positive feedback on a live evaluation with users \cite{MJM12}. Yoon et al. uses UDC data to compare the results obtained from a proposed plug-in for the capture of low-level events, finding a very similar report on command distribution from both tools \cite{YM11}.
...
\label{tbl:p_value}
\centering
\begin{tabular}{l | p{0.75cm} | p{0.85cm} | p{0.85cm} | p{0.85cm} |p{0.75cm}} 
...
\section{Comparison of results with UDC}
...
The edit ratio behaves on a similar manner in comparison with the data on Mylyn, where the median changes accordingly with the number of interruptions, decreasing when there are more and reaching the maximum when there are none. An important difference is that the effect overall is much less pronounced in UDC and we can see it on the effect size tests shown in the Table \ref{tbl:p_value_udc}. The numbers represent the difference on the effect and support our previous judgment. 
...
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_int_events_ratio} \\
\end{tabular}
\caption{Boxplots showing the relation between the number of edits and selection events per minute, the edit ratio, and the number of interruptions. }
...
Again the hypothesis holds as with Mylyn. We can see on the Figure \ref{fig:box_dur_events_udc} that with short interruptions the metrics are greater than when facing a prolonged one, for more time is needed to recover after a long interruption. 
...
The effect size test results on the Table \ref{tbl:p_value2_udc} matches the previous observations on a less pronounced effect. The Cohen's \textit{d} test results tell us that we can label the effect from medium to large and the mean values act accordingly to our hypothesis and conclusions with Mylyn.

\subsection{RQ3: Local relationships between interruptions and productivity}
The sessions with UDC went through the same process of transformation into 10 chunks of equal time. The results on the Figure \ref{fig:bins_int_udc} show the same productivity pattern that we saw before with Mylyn.
\begin{figure}[!ht]
\centering
\begin{tabular}{c}
...
Considering the results obtained on the local analysis of an interruption, we can conclude that before an interruption it's common to have a reduction of both editions and selections per minute. Therefore, the \textit{After Recovery Time} time was split into two phases as well: one that takes the time where the metrics drop which happens 10 minutes before the next interruption (labeled as  \textit{Before Interruption}), and the rest is the peak of the session after the Recovery Time. The duration of the phase \textit{Before Interruption} was chosen with the conclusions after the local analysis around the interruptions.  We can see on the Figure \ref{fig:box_recovery_udc} the evolution of the metrics through the phases.
...
The three metrics show a tendency to increase and reach a maximum between the last phase of the \textit{Recovery Time} and the peak of the session. Moreover, the fall of the productivity on the last minutes before the next interruptions agrees with the observations on the local analysis of the time around an interruption.
