...
\section{Usage Data Collector}

\subsection{Data description}
The Usage Data Collector dataset is a big compendium of information about interaction data from users of Eclipse through December 2008 to August 2010 with the intention to keep record of how programmers are using the IDE. It's a framework that listens to the events triggered by the user or system like edition and navigation commands, the startup of a plug in or the closing of the platform. More specifically, UDC collects information about loaded bundles, commands accessed via keyboard shortcuts, actions invoked via menu or tool-bars, perspective changes, view usage and editor usage. It is a big dataset that contains information of around 1,800,000 users and has a total of 2,323,233,101 unique rows with 5 attributes each. The table \ref{tbl:att_udc} show a description of the attributes.
...
Due to the magnitude of the dataset we only worked on a fragment of it. By doing a random selection of 1,000 users we obtained a good representation of it all and is actually bigger and richer that the dataset we had from Mylyn.
...
After obtaining the data, the next step was to add the attributes required to match it to the data on Mylyn to every row such as the type of the event, the duration and an ID to identify the different sessions on the data. For the latter, we sorted the data by \textit{userId} and by \textit{datetime} afterwards. This was required because by default the user's data is mixed and we needed it not only chronologically correct but also sorted by users to tag the sessions of every user without interferences. 

This dataset doesn't have an attribute to show the time the events ended so we added an interval time in seconds that depends on the next event executed.

Once the events were classified and sorted, we split the dataset into sessions, defining a session as a period of registered work between interruptions of 8 hours or larger, as we did with Mylyn. We obtained a total of 28,989 different sessions but after a filter to keep only those sessions with at least 30 minutes of productive time we ended up with 15,825.
...
The graphs (Figure \ref{fig:box_int_events_udc}) show a similar pattern in comparison with the results with Mylyn. When facing zero interruptions the editions and selections per minute are greater than with one or more interruptions, and the more interruptions the lesser are those metrics.

The edit ratio also behaves on a similar manner in comparison with the Mylyn data but the effect is less pronounced and with more variation on the boxes. Still, the median changes accordingly with the number of interruptions decreasing when there are more in a session.

\begin{figure}[!ht]
\centering
\begin{tabular}{c}
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_int_events_edit}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_int_events_sel}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_int_ratio} \\
\end{tabular}
\caption{Boxplots showing the relation between the number of edits and selection events per minute, the edit ratio, and the number of interruptions. }
...
The next analysis is on the size of the interruptions, considering short and prolonged interruptions.
\begin{itemize}
\item \textit{short}: as with Mylyn we considered those interruptions that only have short sessions ($<$ 12 minutes of duration). They are the 18\% of the total.
...
\centering
\begin{tabular}{c}
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_plot_range_edit}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_plot_range_sel}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_dur_ratio}
\end{tabular}
\caption{Boxplots showing the relation between the number of edits and selections per minute, the edit ratio and interruption duration}
\label{fig:box_dur_events_udc}
\end{figure}

Again the hypothesis holds as with Mylyn. We can see on the Figure \ref{fig:box_int_events_udc} that with short interruptions the metrics are greater than when facing a prolonged one, for more time is needed to recover after a long interruption. 

\subsection{RQ3: Local relationships between interruptions and productivity}
The sessions with UDC went through the same process of transformation into 10 chunks of equal time as with Mylyn. The median of edit and selections are grater in sessions without interruptions, and the editions are more common than selections. When facing at least one prolonged interruption we can see the metrics drop in comparison with short interruptions and the selections are now more frequent than the editions events. This agrees with the results with Mylyn on this point and in the fact that during the beginning and the end of a session both metrics are lower than when in the middle of the session, but in this case the effect is less pronounced. 

\begin{figure}[!ht]
\centering
\begin{tabular}{c}
\includegraphics[width=.80\linewidth,clip=, angle=0]{figures/udc_without_int_med} \\
\includegraphics[width=.80\linewidth,clip=, angle=0]{figures/udc_with_int1_med} \\
\includegraphics[width=.80\linewidth,clip=, angle=0]{figures/udc_with_int2_med} 
\end{tabular}
\caption{Global representation of a session with: no interruptions (top); only short interruptions (middle); and at least one prolonged interruption (bottom). }
\label{fig:bins_int_udc}
\end{figure}

On a local analysis around the interruptions, we can see  interesting behaviors of the edition and selections metrics in a similar manner as with Mylyn. In the Figure \ref{fig:all_ints_udc} we show the frequency of editions and selections around the interruptions, specifically 30 minutes before and after. We can see that close to the interruption the frequency of editions and selections drastically drop, and in a radius of around 10 minutes both metrics are below the overall median. This phenomenon is also visible on the same version of this plot with Mylyn, agreeing that the activity of programming gradually decreases before the interruption and takes 5 minutes or more after it to restore the intensity of work above the median.

\begin{figure}[!ht]
\centering
\begin{tabular}{c}
\includegraphics[width=0.9\linewidth,clip=, angle=0]{figures/udc_all_ints} 
\end{tabular}
\caption{Local effect of an interruption in the user activity.}
...
effect & pattern \\
\hline
\textit{negative} (45\%): when the frequency of edit events decreases after the interruption
...
\textit{neutral} (11\%): when there is no well defined effect before or after the interruption 
...
From those valid interruptions, we split the time after the recovery into two phases: the first 15 minutes labeled as “recovery time” and the rest as “after recovery time”. We calculated the metrics of every recovery and after recovery time, the former divided into three chunks of five minutes to better visualize the evolution of the metrics.

Considering the results obtained on the local analysis of an interruption, we can conclude that before an interruption it's common to have a reduction of both editions and selections per minute. Therefore, the after recovery time was split into two phases: one that takes the time where the metrics drop which happens 10 minutes before it (labeled as  "before interruption"), and the rest is the peak of productivity after the recovery time. We can see on the figure \ref{fig:box_recovery_udc} the evolution of the metrics through the recovery and after recovery phases.

\begin{figure}[!ht]
\centering
\begin{tabular}{c}
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_recovery_edit}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_recovery_sel}  \\
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_recovery_ratio} \\
\end{tabular}
...
We take a closer look into the recovery time by splitting it into interruptions of positive and negative events. As expected the edit ratio is higher during recovery time after a positive interruption and the contrary is seen after a negative interruption.  The figure \ref{fig:box_recovery_effect_udc} show the evolution of the edit ratio during the recovery time, comparing the effects of a positive interruption against a negative one, and the behavior of the edit ratio considering all interruptions.

We can see that after a positive interruption the edit ratio during recovery time is much higher than after a negative interruption, which agrees with the results on the local analysis of interruptions and the effects on the metrics.

\begin{figure}[!ht]
\centering
\begin{tabular}{c}
\includegraphics[width=.8\linewidth,clip=, angle=0]{figures/udc_box_recovery_pos_ratio} \\
\end{tabular}
...
For this we created a new classification of events on a lower level to better identify those that are more frequent during recovery time and according to the nature of the interruption. The new classification contains the categories described on the table \ref{tbl:new_events_udc}.
...
We calculated a weighted average of every event based on the average frequency of execution during the first phase of recovery time after a positive or negative interruption, multiplied by the percentage of users that make use of that event. The same calculation was made using statistics considering all the phases (recovery and non-recovery time). Then we compared the weighted average changes to have evidence of what kind of events are executed after an interruption depending on its effect. The statistics can be found on the table \ref{tbl:stats_events}.
...
After a positive interruption we can see that the editions of text, refactoring and file management are more common and to a lesser extent navigation through the text. On the other hand the navigation around classes, find and replace, searches, debug, clean and build are actions more common after a negative.

We can see that after a positive interruption there are more assertive actions with an intention like refactoring a class or saving all changes. And after a negative interruption there are more uncertainty like debugging, searching for text or navigating around classes.
