...
The Usage Data Collector dataset is a big compendium of information about interaction data from users of Eclipse through December 2008 to August 2010 with the intention to keep record of how programmers are using the IDE. It's a framework that listens to the events triggered by the user and system like edition and navigation commands, the startup of a plug in or the closing of the platform. More specifically, UDC collects information about loaded bundles, commands accessed via keyboard shortcuts, actions invoked via menu or tool-bars, perspective changes, view usage and editor usage. It is a big dataset that contains information of around 1,800,000 users and has a total of 2,323,233,101 unique rows with 5 attributes each. The next table show a description of the attributes.

\begin{table}[ht!]
\renewcommand{\arraystretch}{1.3}
\caption{Attributes in UDC data~\cite{KM06}. }
\label{tbl:kind_event}
\centering
