...
\subsection{Preview Phases for Knowledge Discovery}
We used the phases of the process \textit{Knowledge Discovery in Databases (KDD~\cite{FPP96})} to discover useful knowledge from our collection of data: we first select data, then process it, before transforming it to time series. This section describes on detail the activities performed for every phase. %  (see Figure~\ref{fig:esq}). 
...
\subsection{Preview Phases for Knowledge Discovery}
With this dataset we also followed the phases of the Knowledge Discovery in Databases to discover interesting patterns and responses to the interruptions. Hereafter we describe the selection of the data, processing activities and transformation to a format similar to the Mylyn dataset.

\subsection*{Selection}
Due to the magnitude of the dataset we only worked on a fragment of it. We took a random sample of 1,000 users, which is large enough to be representative; in fact, this is larger than the dataset we were able to obtain for the Mylyn study.

The first step was to query the dataset to get the data from 1,000 random users. We delimited the query to obtain only those events dispatched by the user, ignoring system events, which are those that activate a bundle when starting the IDE, or stop them when closing it. We also ignored the bundle version and formatted the UNIX date into a more legible datetime format. From this query we extracted 4,321,349 unique events, which is around $0.18\%$ of the whole dataset.

\subsection*{Preprocessing}

We used the data pre-processed by Murphy-Hill, that is published on Google BigQuery \cite{SnipesETALASD}. This is an alternative version of the original UDC dataset, that is cleaned and preprocessed, so that the transformation phase is simple and focused on our needs. It was only required to fill black descriptions on the workbench activation and deactivation events.
...
\begin{itemize}
\item \textit{short}: as with Mylyn we considered those sessions that only have short interruptions ($<$ 12 minutes of duration). They constitute 6\% of the total.
...
From those valid interruptions, we split the time after the interruption event into two phases: the first 15 minutes labeled as \textit{Recovery Time} and the rest as \textit{After Recovery Time}. We calculated the edit selection and edit ratio metrics of all the phases, after dividing the Recovery Time into five minute chunks to better visualize the evolution of the metrics. The Figure \ref{fig:recovery_afterRecovery} shows how the time after the interruption was split into phases.

\begin{figure}[!ht]
	\centering
	\begin{tabular}{c}
		\includegraphics[width=.6\linewidth,clip=, angle=0]{figures/udc_after_Interruption} \\
	\end{tabular}
