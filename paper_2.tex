...
Following the conclusions on RQ1, the number of interruptions may affect the results if we ignore that variable. So, we split every group in accordance to the number of interruptions; this time, instead of taking the quantiles of the distribution to create groups we created a group for every number of interruptions up to seven. Since the results in RQ1 show that as the number of interruptions increases the difference between the groups minimizes, we believe that having more distinctive groups instead of groups created by ranges is also valid. Thus, for the three metrics, we created the groups of sessions according to the number on interruptions, between one and seven, en then we split every group in low and high, according to the proportion of prolonged interruptions.

The Figure \ref{fig:dur_mylyn} shows the median of the metrics when grouping the sessions by the number of interruptions and the proportion of prolonged interruptions with Mylyn, and the Figure \ref{fig:dur_udc} with UDC. In this context, it is better to have a low number of interruptions and a low proportion of prolonged interruptions, and the results agree with this statement. First, we observe that (as we conclude in RQ1) when the number of interruption increases the median of the metric decreases until reaching a point (approximately four interruptions) where the change is minimal. Second, almost in every group the median of groups with low proportion of prolonged interruptions is greater than in groups with a high proportion. The latter observation is more clear in the editions per minute metric in both datasets.

%For example, when having between 1 to 3 interruptions and a low proportion the metrics show higher values when compared to having $>$ 5 interruptions and a high proportion of prolonged interruptions. Moreover, if we focus on a category we can see a gradual decrease as the number of interruptions increases, which lead us to conclude that the proportion of prolonged interruptions has an effect on the developer productivity. In the Figures the color of lines represent a classification of the proportion of prolonged interruptions, being the green line the \textit{low} type, blue the \textit{medium} and red the \textit{high}.
...
%Comparing the results between the datasets, UDC seems to show a more clear difference between groups. The sessions with a low proportion of prolonged interruption has a greater difference (higher productivity) than sessions with medium proportion. In Mylyn this difference is more subtle but can be identified, although when having more than 6 interruptions there is not a real difference. We observed that having a high number of prolonged interruptions is more detrimental to productivity, and we can reach the same conclusions with both datasets and with the three metrics.
...
The Tables \ref{tbl:effect_size_dur_mylyn} and \ref{tbl:effect_size_dur_udc} show a test performed in every group of interruptions, measuring the effect of sessions with low and high proportion of prolonged interruptions.
...
\begin{table}[ht!]
	...
	\begin{tabular}{l | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm}} 
		...
		\hline
		U-test (adjusted) & 5.6e-10 & 6.5e-07 & 2.9e-04 & 7.4e-03 & 1.2e-03 & 3.2e-01 &  3.9e-01\\
		\hline
		Cohen's $d$ & 0.62 & 0.38 & 0.20 & 0.08 & 0.21 & 0.17 & 0.45 \\
		\hline
		...
		\hline 
		U-test (adjusted) & 2.6e-06 & 5.7e-05 & 2.3e-02 & 6.4e-01 & 2.3e-02 & 4.9e-01 & 1.5e-01 \\
		\hline 
		Cohen's $d$ & 0.52 & 0.40 & 0.21 & 0.06 & 0.41 & 0.04 & 0.56  \\
		\hline
...
\begin{table}[ht!]
	...
	\begin{tabular}{l | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm}} 
		...
		\multicolumn{8}{c}{\textbf{Edits}} \\
		\hline
		U-test (adjusted) & 1.9e-20 & 4.0e-13 & 1.1e-03 & 2.7e-02 & 7.2e-08 & 5.5e-02 & 6.5e-06\\
		\hline
		...
		\multicolumn{8}{c}{\textbf{Selections}} \\
		\hline 
		U-test (adjusted) & 2.9e-07 & 2.3e-11 & 3.2e-01 & 1.2e-01 & 4.3e-03 & 4.5e-01 & 1.2e-01 \\
		\hline
		...
		\multicolumn{8}{c}{\textbf{Edit ratio}} \\
		\hline 
		U-test (adjusted) & 3.5e-05 & 3.3e-02 & 3.3e-02 & 2.0e-01 & 4.0e-03 & 1.4e-01 & 5.9e-03 \\
		\hline 
