...
While these studies already show the effect of work fragmentation can be hurtful, they are mostly observational studies over a limited time, and do not analyze in detail the use of tools from where additional evidence could be gathered to better understand work fragmentation. In the case of programmers the use of tools to create software is pervasive; integrated software development environments (IDEs) are tools where a significant proportion of the work is done and the interaction with them (or lack thereof) can mark moments of interruption \cite{BK16}.

Similarly, previous studies although they explore the nature of activities of information workers, do not characterize them at a level which can be used to understand particular aspects of programming as a professional activity. According to Parnin and Rugaber \cite{PR12}, programmers must perform activities that challenge the memory, such as keeping track of refactoring changes, navigate and debug unfamiliar code, learn new APIs, and understand new concepts; when these tasks are interrupted, it can be costly to recover the lost state. Some of these tasks require to invest time in program comprehension activities, which can account from 54 to 95\% of the time spent on every working session \cite{MMLK14}. 
...
We also had to fill the description for the events that indicate the activation or deactivation of the workbench of Eclipse, but as they were the only events with this error this task was trivial. Additionally, we needed to classify the events, which was a more complicated task, described below.
...
\begin{changedforreviewerlong}
To ensure that a small subset of users do not dominate our dataset, we counted the number of sessions attributed to each UDC user. The first quartile was 5 sessions, the median 15, and the third quartile 38; the minimum was 1, and the maximum 270. These figures tell us that if some users have more sessions than others (which is to be expected), there is no major bias due to a small number of users concentrating the vast majority of the sessions.
\end{changedforreviewerlong}
...
\includegraphics[width=\linewidth,clip=, angle=0]{figures/box_int_events_edit_2}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/box_int_events_sel_2}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/box_int_ratio_2} 
\endminipage\hfill
\end{tabular}
...
\begin{figure}[!ht]
	...
	\begin{tabular}{c}
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_box_int_events_edit_2}  \\
		\endminipage\hfill
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_box_int_events_sel_2}  \\
		\endminipage\hfil
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_box_int_events_ratio_2} \\
		\endminipage\hfill
	\end{tabular}
...
\begin{tabular}{c}
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_emin_2}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_smin_2}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_eratio_2} 
\endminipage\hfill
\end{tabular}
...
\begin{figure}[!ht]
	...
	\begin{tabular}{c}
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_emin_2}  \\
		\endminipage\hfill
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_smin_2}  \\
		\endminipage\hfil
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_eratio_2} \\
		\endminipage\hfill
	\end{tabular}
...
\begin{table}[ht!]
	\small
	\renewcommand{\arraystretch}{1.3}
	\caption{New classification of events that reflects in detail the nature of each event. }
...
\begin{changedforreviewerlong}
Certainly, work fragmentation is not a synonym of lower productivity. Multitasking can allow the worker to use dead periods of the project where there is a decrease of the required activities and do tasks related to other projects, increasing the efficiency and productivity \cite{ABV12}. At some level, switching between tasks can provide the needed arousal and alertness to improve the productivity, but higher levels of multitasking can be detrimental and cause a negative effect. Moreover, as the number of switches between tasks increases, the effectiveness degrades in a linear fashion \cite{AB12}. However, a study by Monk found that when faced with a higher freqency of interruptions, subjects programming a VCR had a reduced resumption lag, indicating that they adapted to the interruption frequency \cite{monk2004effect}. Also, it was observed that switching between projects might degrade the productivity levels of the programmer due to the context switches, so developers that work on many projects per day have lower productivity \cite{VBX16}.

Our results indicate a clear drop of productivity when facing interruptions: when we increase the number of interruptions, the productivity metrics tend to decrease, but the most dramatic decrease happens for the first interruptions. This agrees with the observational studies cited before, particularly Monk's, even if we can not relate our interruptions to actual multitasking.
\end{changedforreviewerlong}
...
Our theoretical foundations are based on observational studies performed on information workers' development environments \cite{MGH05, IH07, LVD06, PD10}. These findings helped us understand the response to work fragmentation according to the recorded user activity, and it was possible to make a relation between the user's data and the phenomena that occur on the working places described in the studies.
