...
For every session, we calculated the proportion of prolonged interruptions as the sum of interruptions with duration equal or greater than 12 minutes, divided by the total sum of interruptions in that session. Then, we obtained the quantiles of the distribution of the proportions and use them to create 3 groups described in the following:
...
\item \textit{high}: finally, more than half the sessions in the last group are prolonged (proportion $>$0.50). This represents 20.5\% of the sessions in Mylyn and 60.1\% in UDC.
\end{itemize} 
...
Following the conclusions on RQ1, the number of interruptions may affect the results if we ignore that variable. So, we split every group in accordance to the number of interruptions; this time, instead of taking the quantiles of the distribution we took the terciles. Since the results in RQ1 show that as the number of interruptions increases the difference between the groups minimizes, we believe that having four more distinctive groups instead of five is also valid. Thus, for the three metrics, we created the groups low, medium and high, and then we split every group into four additional groups according to the terciles of the distribution of the number on interruptions. 

One of those groups originally consisted of sessions with 1 to 3 interruptions, but we decided to split it into two groups: a group of sessions with one interruption and a second group of sessions with 2 to 3 interruptions. The intention is to see if there is a clear effect between sessions with only one interruption, when this interruption is either short or prolonged. So, we created five groups in total: the \textit{none} group of sessions without interruptions, a group of sessions with only 1 interruption, another group with 2 to 3 interruptions, a fourth group with 4 to 6 interruptions, and lastly a group of sessions with more than 6 interruptions. The value of the terciles turned out to be the same for Mylyn and UDC (30\%: 3 and 60\%: 6) so we kept the same groups in both analyses.
