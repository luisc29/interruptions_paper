...
\begin{changedforreviewerlong}
To ensure that a small subset of users do not dominate our dataset, we counted the number of sessions attributed to each UDC user. The third quartile was 5 sessions, the median 15, and the third quartile 38; the minimum was 1, and the maximum 270. These figures tell us that if some users have more sessions than others (which is to be expected), there is no major bias due to a small number of users concentrating the vast majority of the sessions.
\end{changedforreviewerlong}
