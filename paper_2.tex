...
After the transformation phase, we had the sessions in a proper format to carry out the analysis. In the Figure \ref{fig:mylyn_sessions_stats} we can observe some statistics (left) about the sessions in Mylyn and the distribution (right) of the productive time. The productive time of the session is the total time without considering the time consumed by interruptions. Also, the outliers are ignored by setting a threshold of three times the standard deviation. This filter is used only to show the distribution, to avoid a very long tail provoked by unusually long sessions.
...
At this point, we had the same format of sessions with both datasets. In the Figure \ref{fig:udc_sessions_stats} we can see statistics about the sessions (left) and a histogram (right) of the duration of the sessions on UDC, which ignores outliers by setting a threshold of three times the standard deviation.
