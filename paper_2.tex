...
The metrics that characterize the productivity are related to the amount of activity registered and could be possible to find a correlation between them, specially with the number of edits and selections per minute, and the edit ratio. We did a correlation analysis involving these three metrics (Table \ref{tbl:corr_analysis}, classifying the data between traces with or without interruptions. We find that the Edits/min and Selections/minute metrics have low correlation coefficients, which is the expected result as they explain two different kind of activities. The same happens between the Selections/minute and Edit ratio metrics with the data of both kind of traces, this time being a negative correlation which is also an expected value, as the Edit ratio should decrease when the Selections/minute increases. 

The Edit/minute and Edit ratio correlation coefficient is higher in both groups, but not high enough to not consider the latter, for it tells something different from the user activity. It is normal to see these two metrics with a moderate correlation coefficient given that the Edit ratio is the proportion of edition events in a minute.
...
In this section we analyze whether or not the duration of the interruptions is a factor in the relationship between interruptions and developer productivity. Under the hypothesis that prolonged interruptions require more time to recover, we built two groups of sessions based on the proportion of long interruptions. As we described previously we consider that a prolonged interruption has a duration of $\geq$ 12 minutes, according to previous work \cite{GM04, KaptelininN07}. Prolonged interruptions could be due to an extended interruption, such as a coffee break or a meeting.
...
Following the conclusions on RQ1, the number of interruptions has an impact on the metrics and is a variable that should not be ignored. So, we split every group in accordance to the number of interruptions: this time, instead of taking the quantiles of the distribution to create groups we created a group for every number of interruptions up to seven. Since the results in RQ1 show that as the number of interruptions increases the difference between the groups minimizes, we believe that having more distinctive groups instead of groups created by ranges is also valid. Thus, for the three metrics, we created the groups of sessions according to the number on interruptions, between one and seven, and then we split every group in low and high, according to the proportion of prolonged interruptions.

The Figure \ref{fig:dur_mylyn} shows the median of the metrics when grouping the sessions by the number of interruptions and the proportion of prolonged interruptions with Mylyn, and the Figure \ref{fig:dur_udc} shows the same with UDC. According to our theories, it is better to have a low number of interruptions and a low proportion of prolonged interruptions to have higher rates of productivity, and the results agree with this statement. First, we observe that (as we conclude in RQ1) when the number of interruption increases the median of the metric decreases until reaching a point (approximately between three and four interruptions) where the change is minimal and continues that way for the rest of the groups. Second, almost in every group the median value of groups with low proportion of prolonged interruptions is greater than in groups with a high proportion. The latter observation is more clear in the edits per minute metric in both datasets, for we can see that at some points the medians intertwine in the selections per minutes and edit ratio, specially with Mylyn.

When having a small number of interruptions (between 1 and 3) the difference between sessions with a low or high proportion of prolonged interruptions is more noticeable, and the group of sessions with a low proportion has higher median values. As the number of interruptions increases the differences get smaller, but we observe that the group of low proportion of prolonged interruptions always shows bigger rates. This comments apply on the edits and selections per minute, and to a lesser extent to the edit ratio, where the changes between groups (both by number of interruptions and proportion) are small.
...
When comparing the results between the datasets, we can see the same observations described before. Therefore, we can conclude that the duration of the interruption has an impact when having a low number of interruptions, but when the number is bigger the impact is very subtle. We tested the statistical significance of this results and show the results in the Table \ref{tbl:effect_size_dur_mylyn} for Mylyn, and Table \ref{tbl:effect_size_dur_udc} for UDC. For every group according to the number of interruptions, we measured the effect between the low and high groups of sessions, with the Cohen's $d$ test and U-test. The groups with one to three interruptions have moderate size effects, in comparison with the other groups with more interruptions that have lower size effects; this matches our observations via visual inspection on a more defined difference between the low and high groups when having a small number of interruptions. Starting from (approximately) three interruptions, the distributions of the metrics are very alike, except in some cases like the edit ratio with seven interruptions in Mylyn, where the median is almost the same but the distribution of the values are different enough to have a medium effect size. The bold numbers are those with a significant effect size.
...
\begin{table}[ht!]
	...
	\begin{tabular}{l | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm}} 
		...
		\hline
		U-test (adjusted) & \textbf{5.6e-10} & \textbf{6.5e-07} & \textbf{2.9e-04} & 0.0074 & 0.0021 & 0.32 &  0.39\\
		\hline
		Cohen's $d$ & \textbf{0.62} & 0.38 & 0.20 & 0.08 & 0.21 & 0.17 & 0.45 \\
		\hline
		...
		\hline
		Cohen's $d$ & 0.21 & 0.16 & 0.24 & 0.16 & 0.08 & 0.26 & 0.09  \\  
		\hline
		
		\multicolumn{8}{c}{\textbf{Edit ratio}} \\
		\hline 
		U-test (adjusted) & \textbf{2.6e-06} & \textbf{5.7e-05} & 0.023 & \textbf{0.64} & 0.023 & 0.49 & 0.15 \\
		\hline 
		Cohen's $d$ & \textbf{0.52} & 0.40 & 0.21 & 0.06 & 0.41 & 0.04 & \textbf{0.56}  \\
		\hline
...
\begin{table}[ht!]
	...
	\begin{tabular}{l | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm}} 
		...
		\multicolumn{8}{c}{\textbf{Edits}} \\
		\hline
		U-test (adjusted) & \textbf{1.9e-20} & \textbf{4.0e-13} & 0.0011 & 0.027 & \textbf{7.2e-08} & 0.055 & 6.5e-06\\
		\hline
		Cohen's $d$ & 0.28 & 0.25 & 0.09 & 0.03 & 0.11 & 0.01 & 0.15 \\
		\hline
		
		\multicolumn{8}{c}{\textbf{Selections}} \\
		\hline 
		U-test (adjusted) & \textbf{2.9e-07} & \textbf{2.3e-11} & 0.32 & 0.12 & 0.0043 & 0.45 & 0.12 \\
		\hline
		Cohen's $d$ & 0.23 & 0.26 & 0.01 & 0.09 & 0.09 & 0.07 & 0.06 \\  
		\hline
		
		\multicolumn{8}{c}{\textbf{Edit ratio}} \\
		\hline 
		U-test (adjusted) & \textbf{3.5e-05} & 0.033 & 0.033 & 0.20 & 0.004 & 0.14 & 0.0059 \\
		\hline 
		Cohen's $d$ & 0.31 & 0.13 & 0.10 & 0.06 & 0.17 & 0.09 & 0.19  \\
		\hline
