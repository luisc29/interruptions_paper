...
Work fragmentation is indeed an important phenomenon within the context of modern software development, and the impact there may be even worse since developers build and maintain complex mental models of the software they are working on---these models may be more sensitive to interruptions, and costly to rebuild. %; and 2) development is more often distributed, which may vary the nature of interruptions (trading high-bandwidth meetings for low-bandwidth electronic messages). Achieving flow in these conditions may be challenging.
...
From these research questions, we got the following findings and conclusions:
\begin{itemize}
	\item We found an inverse relationship between the number of interruptions and our productivity metrics.
	\item A high proportion of prolonged interruptions is more detrimental to productivity than short interruptions.
	\item The productivity 30 minutes around an interruption is lower than the average.
	\item There are two well-defined patterns around the interruption, depending if the effect afterwards was positive or negative.
	...
	\item There is a difference between the activities performed after an interruption, depending if it was positive or negative.
\end{itemize}
	...
	\begin{minipage}[c]{0.30\linewidth}
		...
		\centering
			\begin{tabular}{| p{2cm} |p{2cm} |} 
				...
				3rd Qu. & 62 min \\
				max. & 535 min\\
				\changedforreviewer{sd} & 102.36 min\\
				\hline
...
%We filtered these out only in the recovery time analysis

\begin{changedforreviewerlong} 
An important reason to consider the edit ratio is that having an interruption requires doing activities that might increase the execution of selection events, reducing the edit events. For example reading the code, navigating around files and starting a debugging sessions are activities heavily loaded with selections events and are common activities after an interruption, according to observational studies \cite{PR11,PD10}. The edit ratio can tell us when there is a reduction of edit events due to (mainly) selection events.
\end{changedforreviewerlong}

\paragraph{Correlation analysis of the metrics}
%\RR{perhaps expand a bit}
\begin{changedforreviewerlong}
The metrics that characterize the productivity are related to the amount of activity recorded. Thus there is a degree of correlation between them; we study this correlation to determine whether the metrics measure different aspects or not.  %weand could be possible to find a correlation between them, specially with the number of edits and selections per minute, and the edit ratio. 
...
\begin{changedforreviewerlong}
Mylyn events belong to two main categories: edition and selection. The UDC data do not have this, so in order to match the classification of Mylyn we inferred the type of every event manually. So, we do not base our classification on previous work and we are aware that a change or misinterpretation could affect the final results. However, the attributes of the events offer enough information to make an accurate inference in most of the cases.
...
\begin{figure}
	...
	\begin{minipage}[c]{0.30\linewidth}
		...
		\begin{tabular}{| p{2cm} |p{2cm} |} 
			...
			3rd Qu. & 47 min \\
			max. & 1,345 min\\
			\changedforreviewer{sd} & 43.95 min\\
			\hline
...
\begin{tabular}{l | p{0.75cm} | p{1.2cm} | p{1.2cm} | p{1.2cm} |p{1.2cm}} 
  ...
  \hline
  \changedforreviewer{median} & 6.56 & 1 & 0.67 & 0.65 & 0.6	 \\
  \cline{2-6} 
  ...
  \hline 
  \changedforreviewer{median} & 4.56 & 2.08 & 1.71 & 1.42 & 1.28 \\
  \cline{2-6} 
...
\hline
  ...
  \hline 
  \changedforreviewer{median} & 0.56 & 0.36 & 0.30 & 0.32 & 0.33 \\
  \cline{2-6} 
...
\begin{table}[ht!]
	...
	\begin{tabular}{l | p{0.75cm} | p{1.2cm} | p{1.2cm} | p{1.2cm} |p{1.2cm}} 
		...
		\multicolumn{6}{c}{\textbf{Edits}} \\
		\hline
		\changedforreviewer{median} & 3.58 & 1.5 & 1.23 & 1.15 & 1.18 \\
		\cline{2-6} 
		...
		\multicolumn{6}{c}{\textbf{Selections}} \\
		\hline 
		\changedforreviewer{median} & 2.54 & 1.66 & 1.5 & 1.5 & 1.4 \\
		\cline{2-6} 
		...
		\multicolumn{6}{c}{\textbf{Edit ratio}} \\
		\hline 
		\changedforreviewer{median} & 0.57 & 0.49 & 0.45 & 0.43 & 0.44 \\
		\cline{2-6} 
...
Following the conclusions on RQ1, the number of interruptions has an impact on the metrics and should not be ignored. Further, as the number of interruptions increases the difference between the groups minimizes.  %we believe that having   more distinctive groups instead of groups created by ranges is also valid.
...
%We split every group in accordance to the number of interruptions: this time, instead of taking the quantiles of the distribution to create groups we created a group for every number of interruptions up to seven. Since the results in RQ1 show that as the number of interruptions increases the difference between the groups minimizes, we believe that having more distinctive groups instead of groups created by ranges is also valid. Thus, for the three metrics, we created the groups of sessions according to the number on interruptions, between one and seven, and then we split every group in low and high, according to the proportion of prolonged interruptions.
...
%When comparing the results between the datasets, we can see the same observations described before. Therefore, we can conclude that the duration of the interruption has an impact when having a low number of interruptions, but when the number is bigger the impact is very subtle. 
...
\begin{tabular}{c}
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_emin}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_smin}  
\endminipage\hfill
\minipage{0.33\textwidth}
\includegraphics[width=\linewidth,clip=, angle=0]{figures/mylyn_lines_dur_eratio} 
\endminipage\hfill
\end{tabular}
\caption{Median values of edits and selection events per minute, and the edit ratio according to the number of interruptions and proportion of prolonged interruptions in Mylyn. }
\label{fig:dur_mylyn}
\end{figure}
...
\begin{figure}[!ht]
	...
	\begin{tabular}{c}
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_emin}  \\
		\endminipage\hfill
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_smin}  \\
		\endminipage\hfil
		\minipage{0.33\textwidth}
		\includegraphics[width=\linewidth,clip=, angle=0]{figures/udc_lines_dur_eratio} \\
		\endminipage\hfill
	\end{tabular}
	\caption{Median values of edits and selection events per minute, and the edit ratio according to the number of interruptions and proportion of prolonged interruptions in UDC. }
	\label{fig:dur_udc}
\end{figure}



We tested the statistical significance of this results and show the results in the Table \ref{tbl:effect_size_dur_mylyn} for Mylyn, and Table \ref{tbl:effect_size_dur_udc} for UDC. For every group according to the number of interruptions, we measured the effect between the low and high groups of sessions, with the Cliff's delta test and U-test. \changedforreviewer{The groups with one or two interruptions have moderate size effects}, in comparison with the other groups with more interruptions that have lower size effects; this matches our observations via visual inspection on a more defined difference between the low and high groups when having a small number of interruptions. Starting from (approximately) three interruptions, the distributions of the metrics are very alike, except in some cases like the edit ratio with seven interruptions in Mylyn, where the median is almost the same but the distribution of the values are different enough to have a medium effect size. 

\begin{table}[ht!]
	\tiny
	\renewcommand{\arraystretch}{1.3}
	\caption{\changedforreviewer{Effect size and statistical significance of the relationships between the groups of sessions by proportion of prolonged interruptions and the number of interruptions in Mylyn}}
...
\begin{table}[ht!]
	\tiny
	\renewcommand{\arraystretch}{1.3}
	\caption{\changedforreviewer{Effect size and statistical significance of the relationships between the groups of sessions by proportion of prolonged interruptions and the number of interruptions in UDC}}
...
On the other hand, before the interruption there is a preparation phase when the interruption is imminent or expected, e.g. lunch time, a meeting and other scheduled events. In this phase the programmer might leave notes or comments in the code to recover the context after the interruption \cite{PD10}. Also, when the interruption happened because of a problem found by the programmer, the programmer may try to solve it by reading the code, or using the debugger;%or reading comments in the control version tool
...
\begin{changedforreviewerlong}
Table \ref{tbl:local_effect} shows the applied thresholds and the results, accompanied with a typical example of an interruption in each category. This local analysis shows that there are indeed three well-defined groups of interruptions, with the two largest of them having clear effects on the activity in the session. The 45\% of the interruptions are positive in Mylyn, and the 44\% in UDC. The negative interruptions constitute the 44\% in Mylyn and UDC as well. And the neutral interruptions are the 11\% for Mylyn and 12\% for UDC.
\end{changedforreviewerlong}
...
\begin{figure}[!ht]
	...
	\end{tabular}
	\caption{How we split the time after an interruption. }
	\label{fig:recovery_afterRecovery}
\end{figure}

The duration of the peak varies among the phases between interruptions. On average, the peak lasts for 12 minutes, while the maximum registered is of 145 minutes. The distribution of duration of the peaks is 3 minutes for the first quartile, 8 minutes for the second and 16 minutes for the third quartile.  %reported in Table \ref{tbl:dist_peaks}.
...
In RQ3, we saw different patterns of activity around positive and negative interruptions. In particular, we observed that the edits per minute were higher after a positive interruption; on the contrary they were lower after a negative interruption. The effect is even more marked in the \changedforreviewer{first 5 minutes} after the interruption, as can be seen in the Figure \ref{fig:box_recovery_effect_udc}.
...
The presence of this pattern and the important difference observed in Figure \ref{fig:box_recovery_effect_udc} led us to investigate the effect more closely. To this aim, we performed an exploratory study of the activities performed by programmers during the recovery time of an interruption. This is possible due to the high variety of events present in the UDC dataset, which can tell us a lot more about the type of activities that developers do than the Mylyn dataset can. Indeed, the UDC records individual commands that the developers issued in the IDE. The commands are very descriptive in the kind of activity that they represent. 

The goal of the exploratory study was to better understand whether the activity taken after an interruption were indicative of different types of behavior for each type of interruption (positive or negative), and also whether these different kinds of activities supported our earlier hypotheses explaining why the two patterns of interruptions were present. Assuming that different commands represented different types of activity, the assumption was that the frequency of a given activity---as represented by the commands used to carry it---would vary between the different types of sessions.
...
\begin{table}[ht!]
	...
	\begin{tabular}{|p{2.5cm}|p{9 cm}| p{2cm} |}
		\hline 
		category & description & \changedforreviewer{type} \\  
		\hline 
		\hline 
...
category & positive & negative & all \\  
  \hline 
\multicolumn{4}{c}{\textit{\changedforreviewer{Categories that are more frequent after a positive interruption}}} \\
    \hline 
...
perspective & \textbf{0.38} & 0.30 & 0.35  \\
    \hline 
\multicolumn{4}{c}{\textit{\changedforreviewer{Categories that are more frequent after a negative interruption}}} \\
    \hline 
...
clean-build & 0.42 & \textbf{0.62} & 0.56  \\
    \hline 
\multicolumn{4}{c}{\textit{\changedforreviewer{Categories that are less frequent during the recovery time}}} \\
    \hline 
...
\section{Discussion}
\begin{changedforreviewerlong}
The results presented in this work indicate the existence of evidence of the effect of work fragmentation in interaction data, in accordance to what observational studies had concluded before. So, we begin by comparing quantities (number and length of interruptions) and working patterns. The related work to this approach suggests that as a consequence to interruptions there is a recovery time that demands extra time to resume the task, so we expected to see a decrease in the metrics as the number of interruptions increases \cite{MGH05,IH07,CHW04}.

The first research question had the purpose of finding a relationship between the number of interruptions and productivity metrics from user activity, for which we found an inverse relationship between this two variables with both datasets agreeing with the work cited before. This effect seems to be more clear when comparing groups of sessions with a low number of interruptions and the difference is more slight after the fourth or fifth interruption. Also, we observed in the second research question the same inverse relationship when we add the length of interruptions as a third variable, groping the sessions by the proportion of long interruptions.

After this, we not only found evidence of the effect documented in related work, but also gained and reassured the confidence on the metrics we used as indicators of productivity and proceed to look with more detail this effect. So, in the third research question we investigated the productivity 30 minutes around the interruptions, finding a decrease below the median approximately 10 (UDC) to 20 (Mylyn) minutes before and after an interruption. The decrease after an interruption corresponds to the already documented recovery time or resumption time, which could last 15 minutes \cite{IH07, SBV98} depending on the interruption.

We found interesting the similar decrease before the interruption occurs, about 15 minutes before it, that could be due to preparations before the interruption \cite{LVD06} (e.g. taking notes or leaving cues in the code) and/or the programmer's effort when trying to solve the problem that could have caused the interruption \cite{PD10}, which involves reading code, navigating around classes, debugging and more activities. However, Iqbal et al.  \cite{IH07} observed that after an alert the users took between 4 to 7 minutes to respond, and in the meantime they perform self-stabilizing actions before the interruption to have a more efficient recovery time. We can see again this results in the fourth research question, and also a peak of productivity after the recovery time and several minutes before the next interruption.

In the third research question we identified three patterns of interruptions: positive, negative and neutral. We look closer to the activities performed during negative and positive interruptions in the fifth research question, where we set 11 categories of events and calculated the frequency of them after a each type of interruption. We found that assertive activities (e.g. editing, refactoring and saving changes) are more common after a positive interruption, and the comprehension activities (e.g. debugging and navigating around classes) are more common after a negative interruption. We relate the first group of interruptions (in part) to the \textit{bug investigation} workflow described by LaToza et al. \cite{LVD06} and performed by programmers: after they found a problem and are unable to solve it, they consult teammates, documents, emails or artifacts to find a possible solution and then apply it. That would explain the assertive activities.

The second group of interruptions seems to be more disruptive as they require more time to reach the average productivity, and the activities are a reflection of the tasks performed during the recovery time as mentioned by Parnin \cite{PR11}, and they can be matched to the categories high-nav, debug and search, which are the common categories after a negative interruption.
\end{changedforreviewerlong}
...
We classified sessions in groups based on the empirical distribution of the interruptions. In particular, to study the relationship between the number of interruptions and productivity, we split each dataset in four groups based on the quartiles and the median, in order to study the evolution as the number of interruptions grow. We used empirically-defined thresholds since we did not find similar thresholds in the literature. %In the absence of reference from the literature on this topic, basing ourselves on the distribution is the most reasonable choice we could make.
...
%\textbf{
%Interruptions: at least 3 minutes

%Prolonged interruptions: at least12 minutes

%Num. of interruptions: 
%    -mylyn: none, [1-3], [4-5], [6-10], [>11] (minutes)
%     -udc: none, [1-2], [3-4], [5-7], [>7] (minutes

%Proportion of prol. interruptions: 0.50

%Recovery time: 15 min, split into 5 minutes phases

%Before interruption: 10 minutes before the next interruption

%Local analysis: 30 minutes around an interruption

%Filtering sessions: at least 30 minutes of duration

%Creo que la idea era argumentar (en la respuesta a las revisiones) que explorar otras posibilidades en estos thresholds implica mucho trabajo y experimentos, corrígeme si estoy equivocado.
...
%We inspected the effects of work fragmentation in more detail by classifying the interruptions into negative or positive ones, and in the results we can see a difference in productivity and the kind of activities performed.
...
\begin{changedforreviewerlong}
When analyzing the recovery time after an interruption, we filter out the interruptions with less than 30 minutes of activity after it. This means we only use 5\% of the interruptions. We understand that the small sample may put at risk the validity of the results but not all the interruptions are suited for this kind of analysis, as we are looking to a particular case. Also, when we look the activities during the recovery time after an interruption according to the effect (positive or negative), we calculate the frequency of activities (per type) to define when is more frequent. We understand that there could be other interpretations or explanations for this values and the changes, but we back our conclusions with observational studies.
\begin{changedforreviewerlong}
